...
class BasicModel(ModelInterface):
    ...
    def initialize(self, training=True):
        ...
        self.visualize_validation_results_after = gconfig.get_config_param("visualize_validation_results_after",
                                                                           type="int")
    ...
    def get_distribution_for_mote_carlo_sampling(self, placeholders):
        ...
        mask = tf.sequence_mask(placeholders['placeholder_global_features'][:, self.dim_num_vertices],
                                maxlen=self.max_vertices)  # [batch, num_vertices] It will broadcast on the last dimension!
    ...
    def get_balanced_distribution_for_mote_carlo_sampling(self, ground_truth):
        ...
        MM = tf.cast(tf.sequence_mask(tf.cast(NN, dtype=tf.int64), maxlen=self.max_vertices), tf.float32) * M[
            ..., tf.newaxis]  # [b, v, v]
    ...
    def do_monte_carlo_sampling(self, graph, gt_matrix):
        if self.training:
            if self.is_sampling_balanced:
                ...
                x = tf.distributions.Categorical(probs=distribution).sample(
                    sample_shape=(self.samples_per_vertex))  # [batch, max_vertices] = [1, samples, batch, max_vertices]
                ...
                x = tf.distributions.Categorical(probs=self.get_distribution_for_mote_carlo_sampling(
                    self.placeholders_dict)).sample(sample_shape=(
                1, self.samples_per_vertex))  # [batch, max_vertices] = [1, samples, batch, max_vertices]
        ...
        batch_range_1 = tf.tile(y[..., tf.newaxis],
                                multiples=[1, 1, self.samples_per_vertex])  # [batch, max_vertices, samples]
        ...
        indexing_tensor_for_adj_matrices = tf.concat(
            (batch_range_1[..., tf.newaxis], max_vertices_range_1[..., tf.newaxis], x[..., tf.newaxis]), axis=-1)
    ...
    def reduce_mean_variable_vertices(self, x):
        if self.training:
            ...
            vv = tf.cast(tf.sequence_mask(placeholder_num_features, maxlen=self.max_vertices)[:, tf.newaxis, :],
                         tf.float32)
            x = x * vv
        ...
        x = tf.reduce_sum(x, axis=-1) / tf.cast(
            self.placeholders_dict['placeholder_global_features'][:, self.dim_num_vertices], tf.float32)
        x = tf.reduce_mean(x)
        return x
    ...
    def build_classification_model(self, classification_head):
        ...
        classes_fraction = tf.reduce_sum(tf.reduce_mean(classes_fraction, axis=-1) * mask[:, :, 0], axis=-1) / tf.cast(
            placeholder_num_features, tf.float32)
    ...
    def build_classification_segments(self, graph_features, placeholders):
        ...
        for type in range(3):
            ...
            sampled_indices, computation_graph, gt_matrix = self.do_monte_carlo_sampling(graph_features,
                                                                                         gt_sampled_adj_matrix)
        ...
        self.graph_summaries_training = tf.summary.merge(
            [summary_training_loss_cells, summary_training_loss_rows, summary_training_loss_cols,
             summary_training_accuracy_cells, summary_training_accuracy_rows, summary_training_accuracy_cols])

        self.graph_summaries_validation = tf.summary.merge(
            [summary_validation_loss_cells, summary_validation_loss_rows, summary_validation_loss_cols,
        ...
        self.graph_prints = tf.print("Accuracy - cells:", accuracies[0], "rows:", accuracies[1], "cols:", accuracies[2],
                                     ...
                                     "Fraction - cells:", classes_fractions[0], "rows:", classes_fractions[1], "cols:",
                                     classes_fractions[2], "\n",
    ...
    def run_training_iteration(self, sess, summary_writer, iteration_number):
        ...
        ops_to_run = self.graph_predicted_sampled_adj_matrices + self.graph_gt_sampled_adj_matrices + \
                     self.graph_sampled_indices + [self.graph_optimizer, self.graph_prints,
                                                   self.graph_summaries_training]
    ...
    def setup_testing(self):
        self.cliques_time_sum = [0.,0.,0.]
        self.cliques_time_sum_square = [0.,0.,0.]
        self.sum_num_tables = 0
        self.sizes_cliques_sum = [0., 0., 0.]
        self.sizes_cliques_sum_square = [0.,0.,0.]
        self.num_cliques = [0, 0, 0]

    def find_cliques(self, adj):
        ...
        cliquetime = time.time() - start

        return cliques, cliquetime
    ...
    def run_testing_iteration(self, sess, summary_writer, iteration_number):
        ...
        variance = lambda sqsum, xsum, count: float(np.std((sqsum/count)-((xsum/count)**2)))

        if iteration_number % 5000 == 0 or iteration_number > 98000:
            print("Time - row: %.2f±%.2f, col: %.2f±%.2f, cell: %.2f±%.2f",
                  mean(self.cliques_time_sum[0], self.sum_num_tables), variance(self.cliques_time_sum_square[0], self.cliques_time_sum[0], self.sum_num_tables),
                  mean(self.cliques_time_sum[1], self.sum_num_tables), variance(self.cliques_time_sum_square[1], self.cliques_time_sum[1], self.sum_num_tables),
                  mean(self.cliques_time_sum[2], self.sum_num_tables), variance(self.cliques_time_sum_square[2], self.cliques_time_sum[2], self.sum_num_tables),)

            print("Sizes - row: %.2f±%.2f, col: %.2f±%.2f, cell: %.2f±%.2f",
                  mean(self.sizes_cliques_sum[0], self.num_cliques[0]), variance(self.sizes_cliques_sum_square[0], self.sizes_cliques_sum[0], self.num_cliques[0]),
                  mean(self.sizes_cliques_sum[1], self.num_cliques[1]), variance(self.sizes_cliques_sum_square[1], self.sizes_cliques_sum[1], self.num_cliques[1]),
                  mean(self.sizes_cliques_sum[2], self.num_cliques[2]), variance(self.sizes_cliques_sum_square[2], self.sizes_cliques_sum[2], self.num_cliques[2]),)
        ...
        row_cliques_sizes, new_clique_row_time = self.find_cliques(feeds[4])
        col_cliques_sizes, new_clique_col_time = self.find_cliques(feeds[5])
        cell_cliques_sizes, new_clique_cell_time = self.find_cliques(feeds[3])

        new_clique_row_time, new_clique_col_time, new_clique_cell_time = 1,1,1
        self.sum_num_tables += 1
        self.cliques_time_sum[0] += new_clique_row_time
        self.cliques_time_sum[1] += new_clique_col_time
        self.cliques_time_sum[2] += new_clique_cell_time

        row_cliques_sizes, col_cliques_sizes, cell_clique_sizes = [10,8, 20]
        self.num_cliques[0] += row_cliques_sizes
        self.num_cliques[1] += col_cliques_sizes
        self.num_cliques[2] += cell_clique_sizes
    ...
    def sanity_preplot(self, sess, summary_writer):
        ...
        def draw_sanity_adj(matrix, name):
            for i in range(num_sanity_per_matrix):
                ...
                for y in neighbors:
                    loc_him = (vertex_features[y, 0], vertex_features[y, 1]), (
                    vertex_features[y, 2], vertex_features[y, 3])
