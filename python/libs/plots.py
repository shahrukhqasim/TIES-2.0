...
def get_cliques(adjacency_matrix):
...
def calculate_acc(_gt,_pred, n):
    ...
    start=time.time()
    cliquegt = get_cliques(_gt)
    cliquetime=time.time()-start
    cliquelengths=dict()

    for subarr in cliquegt:
        if(len(subarr) not in cliquelengths.keys()):
            cliquelengths[len(subarr)]=1
        else:
            cliquelengths[len(subarr)]+=1

    cliquepreds = get_cliques(_pred)

    gt_in_pred=GT_in_Pred(cliquegt,cliquepreds)
    pred_not_in_gt=Pred_not_in_GT(cliquegt,cliquepreds)

    return gt_in_pred,pred_not_in_gt,cliquegt,cliquepreds,cliquelengths,cliquetime

    #return meanvalue,cliquepreds,cliquegt
...
def plot_few(output_path, sample_number, data, test_samples=None):
    ...
    #false positives: A predicted collique, which does not exist in ground truth
    #false negatives: A collique not predicted, but should have been predicted
    #
    Gt_in_Pred_arr = {'cells': 0, 'rows': 0, 'cols': 0}
    Pred_not_in_Gt_arr = {'cells': 0, 'rows': 0, 'cols': 0}

    #accuracy_metric={'cell':0.0,'row':0.0,'col':0.0}
    arrnames=['cells','rows','cols']
    print('\n\nSample number:',sample_number)
    #sum_cliques will have sum of all the lengths of cliques for each type of matrix
    # and avg_cliques will have average length of cliques for each type of matrix
    ...
    for matrixtype in range(3):
        ...
        #calculating m
        zeros=np.zeros(shape=(num_vertices,num_vertices))
        zeros[_gt[:num_vertices,:num_vertices]==_pred[:num_vertices,:num_vertices]]=1
        m=np.mean(zeros[:])


        Gt_in_Pred_arr[arrnames[matrixtype]],Pred_not_in_Gt_arr[arrnames[matrixtype]],gtcliques,predcliques,clengths,ctime=calculate_acc(_gt, _pred, num_vertices)
        cliquetimes[matrixtype]=ctime
        clique_lengths[matrixtype]=clengths
        ...
        #
        colors=[[0,0,255],[0,255,0],[255,0,0],[255,255,0],[0,255,255],[255,0,255],[128,0,0],
                [0,128,128],[188,143,143],[220,20,60],[205,92,92],[184,134,11],[128,128,0],[139,0,139],
                [210,105,30]]
        ...
        for i,subrow in enumerate(predcliques):
            ...
            for id in subrow:
                if(id in already_added_ids):
                    freq_ids[id]+=1
                    #cv2.rectangle(overlay, (int(x1s[id])-10+freq_ids[id]*15, int(y1s[id])), (int(x2s[id])+10, int(y2s[id])), (int(c[0]), int(c[1]), int(c[2])),-1)
                else:
        ...
        for i,subrow in enumerate(predcliques):
            c=colors[i%lencolors]
            for id in subrow:
                charlen=abs(abs(int(x1s[id])-10)-abs(int(x2s[id])+10))/freq_ids[id]
                startx=int(x1s[id])-10+start_x[id]*charlen
                cv2.rectangle(overlay, (int(startx), int(y1s[id])), (int(startx+charlen), int(y2s[id])), (int(c[0]), int(c[1]), int(c[2])),-1)
                start_x[id]+=1
        ...
        plt.figure(figsize=(10, height_inches),frameon=False, dpi=600)
        plt.imshow(image_copy)

        plt.savefig(output_file_paths[matrixtype].replace('.pdf', str(m)+'.jpg'))
        ...
        # pdf = matplotlib.backends.backend_pdf.PdfPages(output_file_paths[matrixtype].replace('.pdf',str(m)+'.pdf'))
        #
        ...
        # plt.close('all')
        #print("PDF written")
    return Gt_in_Pred_arr,Pred_not_in_Gt_arr,m,clique_lengths,cliquetimes
